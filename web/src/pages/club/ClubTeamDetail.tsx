                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-2">
                            {plantel.map((p, i) => (
                                <div key={i} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 text-center">
                                    ...
                                    {p.atletaUrl ? (
                                        <Link to={`/atleta/${(p.atletaUrl.match(/atletas\/(\d+)/) || [])[1] || ''}?clube=${club.slug}&logo=${encodeURIComponent(club.logo_url || '')}`} className="text-xs font-medium text-zinc-700 dark:text-zinc-300 hover:text-dribly-purple truncate block">
                                            {p.nome}
                                        </Link>
                                    ) : (
