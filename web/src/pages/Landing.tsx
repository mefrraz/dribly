...
function Landing() {
    ...
    const [compMetaMap, setCompMetaMap] = useState<Map<number, {name:string,logo:string|null}>>(new Map())
    ...
    useEffect(() => { const clubPatterns = ['Porto','Benfica','Sporting','Oliveirense']; const orClauses = clubPatterns.map(n => 'equipa_casa.ilike.%' + n + '%,equipa_fora.ilike.%' + n + '%').join(','); supabase.from('games_2025_2026').select('*').or(orClauses).neq('status','FINALIZADO').not('escalao','ilike','%Sub%').gte('data', new Date().toISOString().split('T')[0]).order('data',{ascending:true}).then(({data}:{data:any})=>{if(data){let arr=data as Match[];const counts:Record<string,number>={};arr=arr.filter(m=>{const full=(m.equipa_casa+' '+m.equipa_fora).toUpperCase();const knownTeams=['PORTO','BENFICA','SPORTING','OLIVEIRENSE'];const matchName=['Futebol Clube do Porto','SL Benfica','Sporting Clube Portugal','UD Oliveirense'].find(t=>full.includes(t.toUpperCase()));if(!matchName)return false;const key=knownTeams.find(t=>matchName.toUpperCase().includes(t));if(!key)return false;counts[key]=(counts[key]||0)+1;return counts[key]<=2}).sort(()=>Math.random()-0.5).slice(0,8);setGames(arr)}setGamesLoading(false)}) },[])

    useEffect(() => { if (games.length > 0 && carouselRef.current) { const idx = Math.floor(Math.random() * games.length); carouselRef.current.scrollLeft = idx * 312 } }, [games])
    ...
    useEffect(() => { supabase.from('competitions').select('competition_id, competition_name, association_id, association_name').eq('season','2025/2026').then(({data}) => { if (data) { const seen: Record<number, CompetitionResult> = {}; (data as CompetitionResult[]).forEach(r => { if (!seen[r.competition_id]) seen[r.competition_id] = r }); setAllComps(Object.values(seen)) } }); supabase.from('competitions_meta').select('id, name, logo_url').then(({data:md}) => { if(md) { const cm = new Map(); (md as any[]).forEach((r:any) => cm.set(r.id, {name:r.name, logo:r.logo_url})); setCompMetaMap(cm) } }, () => {}) }, [])
    ...
    return (
            ...
            <div className="relative z-30 bg-gradient-to-b from-dribly-purple/5 via-transparent to-transparent dark:from-dribly-purple/10 dark:via-transparent dark:to-transparent -mt-4 md:-mt-6">
                <div className="max-w-2xl mx-auto px-4 pt-20 md:pt-36 pb-14 md:pb-20 text-center relative">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-dribly-purple/10 dark:bg-dribly-purple/20 text-dribly-purple text-[11px] font-bold uppercase tracking-wider mb-6 "><span className="w-1.5 h-1.5 rounded-full bg-dribly-purple animate-pulse" />Época 2025/2026</div>
                    <h1 className="text-4xl md:text-7xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight mb-2 ">Dribly<span className="text-dribly-purple">.</span></h1>
                    <p className="text-sm md:text-base text-zinc-500 dark:text-zinc-400 max-w-sm mx-auto leading-relaxed mb-6 ">Resultados de todos os clubes de basquetebol em Portugal</p>
                    <div className="flex flex-wrap justify-center gap-2 mb-6 ">{FEATURED_CLUBS.map(({ name, slug }) => { const c = clubs.find(x => x.slug === slug); if (!c) return null; return (<button key={c.slug} onClick={() => selectClub(c)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:border-dribly-purple/30 hover:text-dribly-purple hover:shadow-sm transition-all ${slug === 'ud-oliveirense' ? 'hidden sm:flex' : ''}`}><span className="w-5 h-5 rounded-full bg-zinc-100 dark:bg-white/10 flex items-center justify-center shrink-0 overflow-hidden">{c.logo_url ? <img src={c.logo_url} alt="" className="w-3.5 h-3.5 object-contain" /> : <span className="text-[9px] font-bold text-zinc-500">{name.charAt(0).toUpperCase()}</span>}</span>{displayName(c)}</button>) })}</div>
                    <div className="max-w-lg mx-auto relative " ref={dropdownRef}><div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none"><Search size={20} className="text-zinc-400" /></div><input ref={inputRef} type="text" value={query} onChange={e => { setQuery(e.target.value); setSelectedIdx(-1) }} onKeyDown={handleKeyDown} onFocus={() => query.trim() && setShowDropdown(true)} placeholder="Pesquisar clubes e competições..." className="w-full pl-12 pr-4 py-4 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl text-sm text-zinc-900 dark:text-white placeholder-zinc-400 outline-none shadow-lg shadow-zinc-200/50 dark:shadow-black/20 transition-all focus:ring-2 focus:ring-dribly-purple/30 focus:border-dribly-purple" />{showDropdown && totalResults > 0 && (<div className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl shadow-xl overflow-hidden z-50 text-left">{results.length > 0 && (<div><div className="px-4 py-2 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Clubes</div>{results.map((club,i) => (<button key={club.slug} onClick={() => selectClub(club)} className={'w-full text-left px-4 py-3 flex items-center gap-3 transition-colors ' + (selectedIdx === i ? 'bg-dribly-purple/10 dark:bg-dribly-purple/20' : 'hover:bg-zinc-50 dark:hover:bg-white/5')}><div className="w-9 h-9 rounded-full bg-zinc-100 dark:bg-white/10 flex items-center justify-center shrink-0 overflow-hidden">{club.logo_url ? <img src={club.logo_url} alt="" className="w-6 h-6 object-contain" /> : <span className="text-xs font-bold text-zinc-500">{displayName(club).charAt(0).toUpperCase()}</span>}</div><span className="text-sm font-medium text-zinc-900 dark:text-white truncate">{displayName(club)}</span></button>))}</div>)}{compResults.length > 0 && (<div className={results.length > 0 ? 'border-t border-zinc-100 dark:border-white/5' : ''}><div className="px-4 py-2 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Competições</div>{compResults.map((comp,i) => { const idx = results.length + i; return (<button key={'comp-'+comp.competition_id} onClick={() => { navigate('/competicao/' + comp.competition_id); setQuery(''); setShowDropdown(false) }} className={'w-full text-left px-4 py-3 flex items-center gap-3 transition-colors ' + (selectedIdx === idx ? 'bg-dribly-purple/10 dark:bg-dribly-purple/20' : 'hover:bg-zinc-50 dark:hover:bg-white/5')}>{(()=>{const meta=compMetaMap.get(comp.competition_id);const logo=meta&&meta.logo;return logo?(<div className="w-9 h-9 rounded-full bg-zinc-100 dark:bg-white/10 flex items-center justify-center shrink-0 overflow-hidden"><img src={logo} alt="" className="w-6 h-6 object-contain" /></div>):(<div className="w-9 h-9 rounded-full bg-dribly-purple/10 dark:bg-dribly-purple/20 flex items-center justify-center shrink-0"><Trophy size={16} className="text-dribly-purple" /></div>)})()}<div className="min-w-0"><span className="text-sm font-medium text-zinc-900 dark:text-white truncate block">{compMetaMap.get(comp.competition_id)?.name || comp.competition_name}</span><span className="text-[10px] text-zinc-400">{comp.association_name}</span></div></button>) })}</div>)}{(results.length === 3 || compResults.length === 3) && (<Link to={'/search?q=' + encodeURIComponent(query)} onClick={() => setShowDropdown(false)} className="block w-full text-center py-3 text-xs font-bold text-dribly-purple hover:bg-dribly-purple/5 border-t border-zinc-100 dark:border-white/5 transition-colors">Ver todos os resultados</Link>)}</div>)}</div>

                </div>
            </div>

            {/* Stats */}
            <div className="bg-white dark:bg-zinc-950 border-b border-zinc-100 dark:border-white/5"><div className="px-4 py-4 flex items-center justify-center gap-6 md:gap-16"><div className="text-center"><span className="text-lg font-black text-zinc-900 dark:text-white">{clubs.length}</span><p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-0.5">Clubes</p></div><div className="w-px h-8 bg-zinc-200 dark:bg-white/10" /><div className="text-center"><span className="text-lg font-black text-zinc-900 dark:text-white">{allComps.length || '-'}</span><p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-0.5">Competições</p></div><div className="w-px h-8 bg-zinc-200 dark:bg-white/10" /><div className="text-center"><span className="text-lg font-black text-zinc-900 dark:text-white">{new Set(allComps.map(c => c.association_id)).size || '-'}</span><p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-0.5">Associações</p></div></div></div>

            {/* Jogos em Destaque — gradient fades both sides, wider cards */}
            <div className="py-8">
                <div className="max-w-5xl mx-auto px-4 mb-4"><div className="flex items-center justify-between"><h2 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-dribly-purple animate-pulse" />Jogos em Destaque</h2><div className="flex gap-1"><button onClick={() => scrollCarousel(-1)} className="p-1.5 rounded-full text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-white/5 disabled:opacity-30 transition-colors"><ChevronLeft size={16} /></button><button onClick={() => scrollCarousel(1)} className="p-1.5 rounded-full text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-white/5 disabled:opacity-30 transition-colors"><ChevronRight size={16} /></button></div></div></div>
                <div className="max-w-5xl mx-auto px-4 relative">
                    {gamesLoading ? (<div className="flex gap-3 overflow-hidden">{[1,2,3].map(i => <div key={i} className="min-w-[320px] h-48 rounded-2xl bg-zinc-100 dark:bg-zinc-900 animate-pulse shrink-0" />)}</div>) : games.length === 0 ? (<p className="text-xs text-zinc-400 text-center py-8">Nenhum jogo em destaque de momento.</p>) : (<div className="relative"><div ref={carouselRef} className="flex gap-3 overflow-x-auto pb-2 scrollbar-none snap-x snap-mandatory scroll-smooth items-stretch"><div className="shrink-0 w-[calc(50vw-24px)] md:hidden" />{[...games, ...games].map((match, idx) => (<div key={match.slug || match.id + '-' + idx} className="min-w-[80vw] md:min-w-[320px] shrink-0 snap-center h-full"><GameCard match={match} mode="agenda" /></div>))}<div className="shrink-0 w-[calc(50vw-24px)] md:hidden" /></div><div className="absolute left-0 top-0 bottom-2 w-24 sm:w-32 bg-gradient-to-r from-zinc-50 dark:from-zinc-950 to-transparent pointer-events-none z-10 hidden md:block" /><div className="absolute right-0 top-0 bottom-2 w-24 sm:w-32 bg-gradient-to-l from-zinc-50 dark:from-zinc-950 to-transparent pointer-events-none z-10 hidden md:block" /></div>)}
                </div>
            </div>
